...
class GoogleBot(BaseBot):
    ...
    @measure_step
    async def step_load_google_homepage(self) -> None:
    ...
    @measure_step
    async def step_verify_search_bar(self) -> None:
    ...
    @measure_step
    async def step_execute_searches(self) -> None:
    ...
    @measure_step
    async def search_term(self, term: str) -> None:
        ...
        await asyncio.sleep(2)
        results: list[WebElement] = self.driver.find_elements(By.CSS_SELECTOR, 'div.g')
        if not results:
            raise NoSuchElementException(f"No results found for '{term}'")
        await self.send_run_log(f"Search results retrieved for '{term}'.")

    @measure_step
    async def step_extract_search_results(self) -> None:
